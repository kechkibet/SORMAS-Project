 ...
 */
public class AggregateReportsCreateLayout extends VerticalLayout {
	...
	public AggregateReportsCreateLayout(Window window, UserRight editOrCreateUserRight, Runnable closeCallback) {
	...
	private void save() {
		...
		for (AggregateReportEditForm editForm : editForms) {
			...
			if (deaths > 0 || labConfirmations > 0 || newCases > 0) {
				AggregateReportDto report = AggregateReportDto.build();
				report.setDeaths(deaths);
				report.setDisease(diseaseMap.get(editForm.getDisease()));
				report.setDistrict(comboBoxDistrict.getValue());
				report.setEpiWeek(comboBoxEpiweek.getValue().getWeek());
				report.setHealthFacility(comboBoxFacility.getValue());
				report.setLabConfirmations(labConfirmations);
				report.setNewCases(newCases);
				report.setPointOfEntry(comboBoxPoe.getValue());
				report.setRegion(comboBoxRegion.getValue());
				report.setReportingUser(UserProvider.getCurrent().getUser().toReference());
				report.setYear(comboBoxYear.getValue());
